...
def test_set_port_wrong():
    mod = DummyModule1(DummyModule1.get_gui_config().name)
...
def test_pipe_same_modules():
    mod1 = DummyModule1(DummyModule1.get_gui_config().name)
    with pytest.raises(ValueError):
        Pipe(mod1,mod1,["Port1"])

def test_pipe_same_names():
    mod1 = DummyModule1(DummyModule1.get_gui_config().name)
    mod2 = DummyModule2(DummyModule2.get_gui_config().name)
...
def test_pipe_wrong_type_mod1():
    mod1 = DummyModule1(DummyModule1.get_gui_config().name)
    mod3 = DummyModule3(DummyModule3.get_gui_config().name)
...
def test_n_to_one_module():
    mod1 = DummyModule1(DummyModule1.get_gui_config().name)
    mod2 = DummyModule2(DummyModule2.get_gui_config().name)
    mod4 = DummyModule4(DummyModule4.get_gui_config().name)
